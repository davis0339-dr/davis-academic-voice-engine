...
// This is deliberately the same visible counting rule used by
// public/wordCountCompatibility.js. Academic punctuation (U.S., S&P,
// accounting-based, n.d.-a) must not make the server silently demand dozens
// more words than the editor displays to the researcher.
export function manuscriptWordCount(text) {
  return String(text || "")
    .trim()
    .split(/\s+/u)
...
export function lengthContractSatisfied(candidateText, contract) {
  ...
  return manuscriptWordCount(candidateText) >= Number(contract.minimum_candidate_words || 0);
}
