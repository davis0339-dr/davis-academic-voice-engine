...
// survive revision. Citation handling supports surname-style references and
// institutional/corporate authors, which are common in theses.

const PARENTHETICAL_GROUP = /\(([^()\n]{1,360})\)/g;
const YEAR_TOKEN = /\b(?:18|19|20)\d{2}[a-z]?\b|\bn\.d\.(?:-[a-z])?\b/i;
const CITATION_YEAR_AFTER_COMMA = /,\s*(?:18|19|20)\d{2}[a-z]?\b|,\s*n\.d\.(?:-[a-z])?\b/i;

const NARRATIVE_CITATION =
  /\b([A-Z][A-Za-z'’&.\-]*(?:[ \t]+(?:[A-Z][A-Za-z'’&.\-]*|et|al\.|and|&|of|the|for|in|on)){0,9}[ \t]+\((?:(?:\d{4}[a-z]?|n\.d\.(?:-[a-z])?)(?:\s*,\s*(?:\d{4}[a-z]?|n\.d\.(?:-[a-z])?))*)\))/g;

const NUMBERED_CITATION = /\[\d+(?:,\s*\d+)*(?:-\d+)?\]/g;

// Numeric ranges encode one factual relation. Preserve the relationship, not
// merely the endpoint values. Endpoints are also retained as ordinary numbers
...
function rangeEndpoints(ranges) {
  const out = [];
  for (const range of ranges) {
    const parts = range.match(/\d{1,4}/g) || [];
    out.push(...parts);
  }
  return out;
}
